...
import { Plus, Trash2, Check, X, Download, Search, RefreshCw, Ban } from 'lucide-react'
...
export function Blacklist() {
  const { t } = useLanguage()
  const { domains, loading, addDomain, removeDomain, refresh } = useBlacklist()
  ...
  const typeOptions = [
    { value: 'exact', label: t('blacklist.exact') },
    { value: 'wildcard', label: t('blacklist.wildcard') },
    { value: 'regex', label: t('blacklist.regex') }
  ]
  ...
  const exportDomains = () => {
    ...
    a.href = url
    a.download = `blacklist-${new Date().toISOString().split('T')[0]}.txt`
    a.click()
    ...
    <div className="space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">{t('blacklist.title')}</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <CardTitle className="text-xl">{t('blacklist.title')}</CardTitle>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={exportDomains}>
              ...
              <span className="hidden sm:inline ml-2">{t('blacklist.export')}</span>
            </Button>
            ...
            <Button size="sm" onClick={() => setShowAdd(true)}>
              ...
              <span className="hidden sm:inline ml-2">{t('blacklist.add')}</span>
            </Button>
          </div>
        </CardHeader>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-muted-foreground">
                {search ? t('queryLog.noResults') : t('blacklist.empty')}
              </p>
              {!search && (
